...
**⭐本轮(7-22 续)已做+验证**:
- ✅**约面回调真正提速(最高优先级)**:豆包判岗位(~9s)从建档主路径挪到后台线程(rules._enrich_candidate),记录**秒建<100ms**不再等9秒;岗位/岗位大类/渠道/面评标题/简历解析全滞后 patch,符合玄玄"先出基本信息不挡记录"底线。**本地 mock 单测通过**(scratchpad/test_async_sync.py:建档只含基本信息、耗时0ms、后台回填标准岗位+面评标题岗位对齐)。
- ✅**修真根因(建两条)**:`_do_sync` 里 `synced.add(cid)` 引用未定义名(synced 只是 rule1_sync 局部变量)→真实建档必 NameError→ok 到不了 True→`_finish` 不落白名单→抢占锁形同虚设。这才是"刘云昊建两条"真根因(之前误归因纯 race)。已删,白名单由 `_finish(cid,ok=True)` 落盘。
- ✅**改期同步移动日历日程(#12)**:触达侧 `notifyTimeChange` 里,面试官改一面时间→若已建过日程(scheduleEventId)→`feishu.updateInterviewEventTime` PATCH 日历事件到新时间。tsc 通过(未 E2E,缺真实 Mongo/日历)。
- ✅**#10 主动打招呼**:现有防护已挡住给已约成候选人重复打招呼(`sendWelcome` 排除 INTENT_ACCEPT/WELCOMED/REPLIED,沉默扫描器 sweepIdle 只扫 WELCOMED/INTENT_RESCHEDULE 不扫 INTENT_ACCEPT)。唯一残留主动消息=改期沉默时每日回访一句,是否去掉待玄玄定(见下)。

**⛔外部卡点(不是玄玄能定,要宏佳/开权限)**:
- **#11 备注改姓名+岗位**:秒回服务只有 addFriendByPhone/sendText,**无改备注接口**。要宏佳给秒回"改备注"API 文档才能做。
- **#14 逐字稿自动收集**:卡已知问题#3(飞书不让机器人指定会议主持人→妙记文字稿归属不定),且要首场真实面试验证。妙记搜索方法还没集成进 feishu.py。
- **#15 日程编辑权限**:要飞书管理员给应用开通讯录权限(已知问题#5)。

**❓待玄玄拍板(2026-07-22 已用 AskUserQuestion 问,他离开未答)**:①改建日程逻辑=候选人确认才建(现状/稳妥)vs AI触达就建(原文/但给没确认的人建会议爽约要删)②"定时发好友3小时"=未通过每3h重发/首次加好友3h批次节流/养鱼号拿简历就加(二期)——涉及企微账号风控,不敢猜③改期沉默候选人=不主动发只等超时转人工(对齐"别主动联系")vs 每日回访一句(现状)④转人工要不要做成秒级(需玄玄去飞书进度表【转人工】字段配一条自动化 webhook,像约面那样)。**需求原文飞书文档 `F8C1dAxb0oPHTgxITlFcP1GAnYb`(14:00/11:00/7-21三批,含图)**。

**#13 表格↔中台状态同步**:需玄玄说清具体哪不一致(AI触达/转人工勾选?)再动。**#17 转人工**:现已在5s快循环(非慢循环),不算慢;秒级需②里的自动化 webhook。
...
**⭐⭐部署分工定稿(2026-07-19,玄玄)**:**Claude/表格服务方只改代码+commit push;所有部署(三个服务)由宏佳执行**。改完代码→编译测试过→推仓库→群里说一声让宏佳部署。不再rsync服务器、不再docker build/run(紧急故障例外,动手前群里说)。
