...
def rule9_interview_eval(records):
    ...
    for r in records:
        ...
        if not shell.strip():
            # 知识库「AI面评Prompt」读失败时 prompt_shell 静默返回 ''：判断原则/写作风格/分数规则/
            # 固定输出结构全没了,只剩空 JSON 壳——模型照样出分,但质量无声塌方且看不出来。
            # 宁可这轮不评(不标 evaled),下轮知识库恢复了自动重试。
            log.warning(f"规则⑨ {name}：知识库面评Prompt读不到(壳为空)，本轮跳过不评，下轮重试")
            continue
        ...
        prompt = (f"{shell}\n【当前岗位配置】\n{rubric or '(无专属配置,按岗位所属类别默认体系)'}\n\n"
                  ...
                  # 不引用知识库的章节号/标题(改章节号会静默失配);字段清单直接写在下面,
                  # 判断原则/写作风格/条数字数/分数规则仍全部以上面知识库 prompt 为准。
                  "按上面的规范评这场一面。**只输出 JSON**(不要任何其它文字),字段如下:\n"
