...
配置(env,放服务器 .env,绝不进仓库):
  ...
  FEISHU_OAUTH_REDIRECT   在开放平台配的重定向 URL(拿授权码用),缺省 http://localhost:8080/callback
                          (实测:api-explorer/loopback 会被调试台吞掉 code;localhost 页面打不开但 code 停在地址栏)
令牌落盘:data/user_token.json (0600 原子写;refresh_token 长期有效、过期前自动刷新)。
所需用户 scope(开放平台"用户身份"那栏勾上):docx:document:readonly、docs:document.content:read、search:docs:read、offline_access。
"""
...
class FeishuUser:
    def __init__(self, token_path=None):
        ...
        self.redirect = os.environ.get("FEISHU_OAUTH_REDIRECT", "http://localhost:8080/callback")
        self.token_path = token_path or os.path.join(
            os.environ.get("DATA_DIR", "data"), "user_token.json")
        self._lock = threading.RLock()   # token 刷新加锁,防 webhook 线程与主循环并发双刷
    ...
    def _save(self):
        ...
        fd = os.open(tmp, os.O_WRONLY | os.O_CREAT | os.O_TRUNC, 0o600)  # 私有权限,含 refresh_token
        with os.fdopen(fd, "w") as fp:
            json.dump(self._store, fp, ensure_ascii=False)
        os.replace(tmp, self.token_path)  # 原子替换,中断不会截断成半文件
    ...
    def token(self):
        with self._lock:  # RLock:与 refresh() 可重入;串行化刷新防并发双刷
    ...
    def find_transcript(self, name, position="", interview_ms=None):
        """找某候选人这场面试的文字记录 → (title, token, url) 或 None。
        标题形如「文字记录:线上面试-{岗位}-{姓名} {YYYY年M月D日}」(岗位可能没有)。
        防串档三重校验：①姓名必须是完整一段(边界=分隔符/空白/结尾,不是子串——'张玉'不命中'张玉婷')
        ②标题日期要接近【一面时间】±3天(挡多轮/改期串档)③命中多条(同名等)→放弃并告警,绝不猜。"""
        import re
        hits = self.search_docs(f"文字记录 线上面试 {name}", count=20)
        cands = []
        for h in hits:
            t = h.get("title", "")
            if not (t.startswith("文字记录") and "线上面试" in t):
                continue
            if not re.search(r"[-:：]" + re.escape(name) + r"(\s|$)", t):  # 姓名整段,挡子串串档
                continue
            if interview_ms:  # 标题日期接近一面时间才算(挡多轮/改期/同名不同场)
                md = re.search(r"(\d{4})年(\d{1,2})月(\d{1,2})日", t)
                if md:
                    import datetime
                    try:
                        doc_ms = int(datetime.datetime(*map(int, md.groups())).timestamp() * 1000)
                        if abs(doc_ms - interview_ms) > 3 * 86400 * 1000:
                            continue
                    except Exception:
                        pass
            cands.append(h)
        if len(cands) > 1 and position:  # 多条时用岗位消歧
            pref = [h for h in cands if position in h.get("title", "")]
            if len(pref) == 1:
                cands = pref
        if not cands:
            return None
        if len(cands) > 1:
            log.warning(f"逐字稿多命中({name})放弃避免串档: " + " | ".join(h.get("title", "") for h in cands))
            return None
        h = cands[0]
        return h["title"], h["token"], f"https://juzihudong.feishu.cn/docx/{h['token']}"
